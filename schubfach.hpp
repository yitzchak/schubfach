...
template <typename uint_t> struct math {
  ...
  using uint_2_t = std::conditional_t<limits::digits <= 32, uint64_t, uint64_2_t>;
...
template <> uint64_t math<uint64_t>::round_to_odd(uint64_2_t g, uint64_t cp) {
  ...
  const __uint64_t y0 = static_cast<uint64_t>(y);
  const __uint64_t y1 = static_cast<uint64_t>(y >> limits::digits);
  return y1 | (y0 > 1);
}
